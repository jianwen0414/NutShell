...
function positionFromPlan(
  p: ExecuteHedgeParams,
  plan: ExecutionPlan,
  extras: Partial<HedgePosition>,
): ExecutedPosition {
 ...
 */
export async function executeHedge(p: ExecuteHedgeParams): Promise<ExecutedPosition> {
  const cid = p.correlationId;
 ...
 */
async function broadcast(p: ExecuteHedgeParams, plan: ExecutionPlan): Promise<ExecutedPosition> {
  const cid = p.correlationId;
 ...
 */
/**
 * A position's execution plan, or a clear refusal.
 *
 * `HedgePosition.execution` is optional in the shared contract, because a
 * fixture or a chain-reconstructed record honestly has none. The lifecycle
 * functions genuinely need one, so they say so rather than reading through
 * `undefined` and failing somewhere less obvious.
 */
function requireExecution(position: HedgePosition): ExecutionPlan {
  if (!position.execution) {
    throw new AppError(
      'VALIDATION_FAILED',
      `Position ${position.correlationId} carries no execution plan. Rebuild it from chain with ` +
        'scripts/import-position.ts, or pass the position returned by executeHedge().',
...
export async function abandonPosition(
  ...
  const execution = requireExecution(position);
  return {
    ...position,
    status: 'UNWOUND',
...
export async function settlePosition(
  ...
  const execution = requireExecution(position);
  const order = execution.selectedOrder;
  const decimals = order.collateralDecimals;
