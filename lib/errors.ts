 ...
 *
 * ## On the two constructor shapes
 *
 * M1 and M2 arrived at the same class with different signatures. Rather than
 * rewrite a dozen call sites across three people's code — the kind of churn
 * that turns an integration into a bug hunt — `AppError` accepts BOTH:
 *
 *   new AppError(code, message, { correlationId, details, cause })   // M1
 *   new AppError(code, message, details, correlationId)              // M2
 *
 * and `toEnvelope()` takes an optional correlation id, so both
 * `err.toEnvelope()` and `err.toEnvelope(jobId)` work. `asAppError` and
 * `toAppError` are aliases of one another for the same reason.
 */
...
/** Distinguish M1's options object from M2's bare details object. */
function isOptions(v: unknown): v is AppErrorOptions {
  if (typeof v !== 'object' || v === null) return false;
  return 'correlationId' in v || 'details' in v || 'cause' in v;
}
...
export class AppError extends Error {
  ...
  constructor(
    code: ErrorCode,
    message: string,
    optionsOrDetails?: AppErrorOptions | Record<string, unknown>,
    correlationIdArg?: CorrelationId,
  ) {
    const opts: AppErrorOptions = isOptions(optionsOrDetails)
      ? optionsOrDetails
      : { details: optionsOrDetails as Record<string, unknown> | undefined };

    super(message, opts.cause === undefined ? undefined : { cause: opts.cause });
    this.name = 'AppError';
    this.code = code;
    const entry = ERROR_REGISTRY[code] ?? { http: 500, retryable: false };
    this.http = entry.http;
    this.retryable = entry.retryable;
    this.correlationId = correlationIdArg ?? opts.correlationId ?? newCorrelationId();
    this.details = opts.details;
    Error.captureStackTrace?.(this, AppError);
  }

  /** `correlationId` overrides the one carried on the error, when given. */
...
/** M2's name for `toAppError`. Kept so both call sites read naturally. */
export const asAppError = toAppError;
 ...
 * decimal string. Use this before ANY value crosses a JSON boundary —
 * PRD §7 invariant 2.
 *
 * A `DecodedOrder` carries the untouched SDK object on `.raw`, which is full
 * of bigints, so `JSON.stringify(order)` throws. Every API route that returns
 * an order or a position must pass it through here first.
 */
